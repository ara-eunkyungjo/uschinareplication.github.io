...
jsPsych.plugins['demog-free-response-audio'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'demog-free-response-audio',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      html += question.required ? ' <font color="brown">*</font>' : '';
    ...
    display_element.querySelector('#jspsych-survey-text-form').addEventListener('submit', function(e) {
      ...
      for(var index=0; index < trial.questions.length; index++){
        ...
        var name = trial.questions[index].name;
        if(name == ''){
          name = id;
        }        
        var obje = {};
