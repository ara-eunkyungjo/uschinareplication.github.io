...
jsPsych.plugins['demog-multi-select-audio'] = (function() {
  var plugin = {};

  plugin.info = {
    name: 'demog-multi-select-audio',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      display_element.querySelector(question_selector).innerHTML += '<p id="survey-question" class="' + plugin_id_name + 
      '-text survey-multi-select">' + question.prompt + reqStar + " " + audioHtml + '</p>';
