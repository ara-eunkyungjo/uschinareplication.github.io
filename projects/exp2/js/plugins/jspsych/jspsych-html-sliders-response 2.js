/**
 * jspsych-html-sliders-response
 *
 * a jspsych plugin for free response to HTML prompts. 
 ...
 * accomplish this.
 *
 * adapted from Matt Jaquiery - https://github.com/mjaquiery/ - Feb 2018
 *
...
jsPsych.plugins['html-sliders-response'] = (function() {

    let plugin = {};

    plugin.info = {
        name: 'html-sliders-response',
        description: 'Collect multiple slider responses to HTML stimuli ',
        parameters: {
            stimulus: {
            ...
            min: {
                ...
                'Format is an array of length slider_count. '+
    ...
    plugin.trial = function(display_element, trial) {
        ...
        var audioHtml = trial.audio ? ' <img role="button"  src="media/images/demog/audio.png" style="height:25px; width:25px"  class="AR" onclick="playAudio(\'' + trial.audio + '\')"/>' : '';
        var reqStar = trial.required ? ' <font color="brown">*</font>' : '';
        var html = '<div id="jspsych-html-slider-response-wrapper" style="margin: 20px 0px;">';
        html += '<div id="jspsych-html-slider-response-stimulus">' + trial.stimulus + reqStar + audioHtml + '</div>';
        ...
        //     trial.canvasId = 'jspsych-html-sliders-response-canvas';
        //     canvas = '<canvas id="'+trial.canvasId+'" height="'+trial.canvasHeight+
        //         '" width="'+trial.canvasWidth+'"></canvas>';
        // }
        // let html = '<div id="jspsych-html-sliders-response-wrapper" class="jspsych-sliders-response-wrapper">';
        // html += '<div id="jspsych-html-sliders-response-stimulus">'+canvas+'</div>';
        // // Prompt text
        ...
        for (let i=0; i<trial.slider_count; i++) {
            sliders.push({
            ...
            for(let s=0; s<sliders.length; s++) {
                ...
                if (max_slider_row === null || slider.arrangement === i) {
                    ...
                    html += '<tr class="jspsych-sliders-slider">'+
                        spacer+
                        '<td colspan="'+(colSpan*2-2).toString()+'" //style="width: '+(tdWidth*colSpan).toString()+'%;" '+
                        'class="jspsych-sliders-slider" id="jspsych-html-sliders-response-sliderCell'+s+'"'+
                        ' ><input type="range" style="width:100%" value="'+slider.start+
                        '" min="'+slider.min+'" max="'+slider.max+
                        '" step="'+slider.step+'" '+
                        'id="jspsych-html-sliders-response-slider'+s+'" '+
                        'class="jspsych-sliders-response-slider"/></td>'+
                        spacer+
                        '</tr>';
                    // labels
                    html += '<tr id="jspsych-html-sliders-response-labels'+s
                    ...
                    for(let j=0; j < slider.labels.length; j++){
                        ...
                        html += '<td colspan="2" style="width: '+(tdWidth*2).toString()+
                            ...
                            ' id="jspsych-html-sliders-response-labelS'+s+'L'+j+'">';
        ...
        html += '<div id="jspsych-html-sliders-response-warnings"></div>';

        // add submit button
        html += '<button id="jspsych-html-sliders-response-next" class="jspsych-btn">'+trial.button_label+'</button>';
        ...
        function onSliderChange() {
            ...
            for (let i=0; i<sliders.length; i++) {
                let id = 'jspsych-html-sliders-response-slider'+i;
                if (id===this.id) {
                    slider = sliders[i];
        ...
        function swapSliderChangeFunction(remove=false) {
            for (let i=0; i<sliders.length; i++) {
                let s = display_element.querySelector('#jspsych-html-sliders-response-slider'+i);
                if (remove) {
        ...
        display_element.querySelector('#jspsych-html-sliders-response-next').addEventListener('click', function() {
            ...
            // Issue the warnings
            if (halt) {
                display_element.querySelector('#jspsych-html-sliders-response-warnings').innerHTML = warn_html;
                return;
            }
            ...
            let answers = [];
            for (let i=0; i<sliders.length; i++) {
                let value = display_element.querySelector('#jspsych-html-sliders-response-slider'+i).value;
                let slider = sliders[i];
            ...
            if(trial.response_ends_trial){
                ...
                display_element.querySelector('#jspsych-html-sliders-response-next').disabled = true;
            }
        ...
        if (trial.stimulus_duration !== null) {
            jsPsych.pluginAPI.setTimeout(function() {
                display_element.querySelector('#jspsych-html-sliders-response-stimulus').style.visibility = 'hidden';
