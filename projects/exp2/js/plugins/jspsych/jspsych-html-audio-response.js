 ...
 * modified to permit recording end by either button press or timeout
 *
...
jsPsych.plugins["html-audio-response"] = (function() {
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            recording_light_off: {
                ...
                default: '<div id="jspsych-html-audio-response-light" '+
                'style="border: 2px solid grey; background-color: grey; '+
                'width: 50px; height: 50px; border-radius: 50px; margin: 20px auto; '+
                'display: block;"></div>',
            ...
            retry_button: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Retry button label',
                default: 'Retry',
                description: 'The text that appears on the button to retry recording.'
            },
            next_button: {
                type: jsPsych.plugins.parameterType.STRING,
                pretty_name: 'Next button label',
                default: 'Next',
                description: 'The text that appears on the button to move on to the next trial.'
            }
        }
    };
    ...
    plugin.trial = function(display_element, trial) {
        ...
        html += '<div id="jspsych-html-audio-response-buttons"><button id="jspsych-html-audio-response-rerecord" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">'+trial.retry_button+'</button>'+
            '<button id="jspsych-html-audio-response-okay" class="jspsych-audio-response-button jspsych-btn" style="display: inline-block; margin:'+trial.margin_vertical+' '+trial.margin_horizontal+'; visibility:hidden;">'+trial.next_button+'</button></div>';

        // add done button element
        html += '<button id="jspsych-html-audio-response-done" class="jspsych-audio-response-button jspsych-btn">'+trial.button_label+'</button>'
        ...
        function start_trial() {
            ...
            if (trial.audio) {
                document.querySelector('#jspsych-html-audio-response-recording-container').innerHTML = trial.recording_light_off;
            } else {
            ...
            } else if (!trial.wait_for_mic_approval) {
                start_recording();
            }
        }
        ...
        function start_recording() {
            ...
            let done_btn = document.querySelector('#jspsych-html-audio-response-done')
            if (done_btn != null) { done_btn.style.visibility = "visible" }
        ...
        function process_audio(stream) {
            ...
            function handlerFunction(stream) {
                ...
                if (trial.buffer_length > 0) {
                    // setTimeout to stop recording after buffer_length seconds
                ...
                // also stops recording when done button is clicked
                display_element.querySelector('#jspsych-html-audio-response-done').addEventListener('click', function(){
                    if (recorder.state != "inactive") { recorder.stop(); }
                    recorder.wrapUp = true;
                    if (trial.buffer_length > 0) { clearTimeout(trialTimeout); }
                    display_element.querySelector('#jspsych-html-audio-response-done').style.visibility = "hidden";
                })
            } catch(error) {
